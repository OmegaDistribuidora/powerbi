...
function FilterPanel(props) {
  ...
  return <section className="gv-filters">
    ...
    <div className="gv-filter-grid">
      <label><span>Coordenador</span><details className={`gv-coordinator-select ${!props.coordinatorEnabled ? "disabled" : ""}`}><summary onClick={(event) => { if (!props.coordinatorEnabled) event.preventDefault(); }} aria-disabled={!props.coordinatorEnabled}>{props.selectedCoordinator ? props.coordinators.find((user) => user.code === props.selectedCoordinator)?.label || "Todos" : "Todos"}<ChevronDown size={15} /></summary><div className="gv-coordinator-menu"><button type="button" className={!props.selectedCoordinator ? "selected" : ""} onClick={(event) => { props.chooseCoordinator(""); event.currentTarget.closest("details")?.removeAttribute("open"); }}>Todos</button>{props.coordinators.map((user) => <button type="button" key={user.id} className={props.selectedCoordinator === user.code ? "selected" : ""} onClick={(event) => { props.chooseCoordinator(user.code); event.currentTarget.closest("details")?.removeAttribute("open"); }}>{user.label}</button>)}</div></details><small>{!props.coordinatorEnabled ? "Indisponível para o perfil escolhido." : ""}</small></label>
